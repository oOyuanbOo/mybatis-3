...
class BindingTest {
  ...
  static void setup() throws Exception {
    // 配置数据源，整到这里我才知道还有好多没看呢   苍天呢，不说了，上去干！
    // 执行表结构，插入数据
    // 看到这里，只知道spring管理的数据源，才发现原来数据源可以这样获取
  ...
  void shouldSelectBlogWithPostsUsingSubSelect() {
    try (SqlSession session = sqlSessionFactory.openSession()) {
      // 通过test构建的第一条测试方法顺利执行，这个流程不出意外我是跑过的
  ...
  void shouldExecuteBoundSelectMapOfBlogsById() {
    try (SqlSession session = sqlSessionFactory.openSession()) {
      BoundBlogMapper mapper = session.getMapper(BoundBlogMapper.class);
      // 这里面有个mapKey的注解，可以关注下，就是把返回值作为entrySet的value，指定字段作为key
  ...
  void shouldExecuteMultipleBoundSelectMapOfBlogsByIdInSameSessionWithoutClearingLocalCache() {
    try (SqlSession session = sqlSessionFactory.openSession()) {
      ...
      // 应该是在测试缓存，断点可以关注是不是走了缓存，名字可以看到，sameSession，就是一级缓存了
      assertEquals(2, blogs.size());
  ...
  // 测试目的看方法名称，虽然臭又长，但是很详细，基本都是在和注解打交道
  @Test
